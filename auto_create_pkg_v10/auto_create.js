...
async function solveCaptcha(page, nopecha, accountNum) {
    ...
    try {
        // Wait for CAPTCHA modal or awswaf-captcha element
        log('INFO', 'Waiting for CAPTCHA...', accountNum);
        let captchaFound = false;
        try {
            await page.waitForSelector('#captchaModal', { timeout: 8000, state: 'attached' });
            captchaFound = true;
        } catch (e) {
            // Try awswaf-captcha as fallback
            try {
                const waf = await page.$('awswaf-captcha');
                if (waf) {
                    captchaFound = true;
                    log('INFO', 'Found awswaf-captcha element (no #captchaModal)', accountNum);
                }
            } catch (e2) {}
        }
        if (!captchaFound) {
...
async function createAccount(browser, config, accountData, accountNum, proxies, nopecha) {
    ...
    try {
        ...
        // Step 5c: Check for phone validation errors and retry with new number
        for (let phoneRetry = 0; phoneRetry < 3; phoneRetry++) {
            const phoneError = await page.evaluate(() => {
                const errorEls = document.querySelectorAll('[role="alert"], .error-message, [class*="error"], .field-error, .form-error');
                const errorTexts = Array.from(errorEls)
                    .filter(el => el.offsetParent !== null)
                    .map(e => e.textContent.toLowerCase()).join(' ');
                return (errorTexts.includes('phone') || errorTexts.includes('mobile') ||
                        errorTexts.includes('number') || errorTexts.includes('invalid')) &&
                       !errorTexts.includes('email') && !errorTexts.includes('already')
                    ? errorTexts.substring(0, 200) : null;
            });

            if (!phoneError) break;

            const newPhone = generateRandomPhone();
            log('INFO', `Phone error: "${phoneError}" — retrying with new number: ${newPhone} (attempt ${phoneRetry + 1}/3)`, accountNum);

            // Re-fill phone fields
            ...
            await fillField(page, 'confirmPhone', newPhone,
                ['Re-enter mobile number', 'Re-enter mobile'],
                ['Re-enter the mobile number', 'Re-enter mobile'],
                ['input[name="confirmPhone"]', 'input[name="reenterPhone"]', 'input[name="confirmMobile"]', 'input[name="reenterMobileNumber"]'],
                accountNum);

            // Also try filling any empty tel inputs
            try {
                const telInputs = await page.$$('input[type="tel"]');
                for (const tel of telInputs) {
                    if (await tel.isVisible()) {
            ...
            // Re-submit
            log('INFO', 'Re-submitting with new phone number...', accountNum);
            let resubmitted = false;
            for (const sel of ['button:has-text("Continue")', 'button:has-text("Create account")', 'button:has-text("Create Account")', 'button:has-text("Submit")', 'button[type="submit"]']) {
                try {
                    const btn = await page.$(sel);
                    if (btn && await btn.isVisible() && await btn.isEnabled()) {
                        await btn.click({ timeout: 5000 });
            ...
            if (!resubmitted) await clickContinueButton(page, accountNum);
            await page.waitForTimeout(3000);
        }
        ...
        for (let i = 0; i < 5; i++) {
            ...
            // But only solve if actually active (overlay visible, height > 50px)
            const captchaActive = await page.evaluate(() => {
                const waf = document.querySelector('awswaf-captcha');
                if (waf) {
                    const rect = waf.getBoundingClientRect();
                    if (rect.height > 50) return 'awswaf';
                }
                const modal = document.querySelector('#captchaModal');
                if (modal) {
                ...
                if (overlay) {
                    const style = window.getComputedStyle(overlay);
                    if (style.display !== 'none' && style.visibility !== 'hidden' && style.pointerEvents !== 'none') return 'overlay';
                }
                return null;
            });
            if (captchaActive) {
                log('INFO', `CAPTCHA active at step ${i + 1} (${captchaActive}), solving...`, accountNum);
        ...
        // Look for OTP input field — use multiple detection strategies
        const otpSelectors = [
            'input[name="otp"]',
        ...
        // Strategy 1: Check explicit OTP selectors
        ...
        // Strategy 2: Page text detection — look for "verification code" / "OTP" text
        if (!otpFieldFound) {
            try {
                const pageText = await page.evaluate(() => document.body.innerText.toLowerCase());
                const hasOtpText = pageText.includes('verification code') || pageText.includes('enter the code') ||
                                   pageText.includes('enter code') || pageText.includes('one-time') ||
                                   pageText.includes('otp') || pageText.includes('verify your');
                if (hasOtpText) {
                    // Count visible inputs — OTP page has 1-4 inputs, registration has 10+
                    ...
                    if (inputCount >= 1 && inputCount <= 4) {
                        otpFieldFound = true;
                        log('INFO', `OTP page detected via text + ${inputCount} visible inputs`, accountNum);
                    }
                }
            } catch (e) {}
        }
        ...
        if (!otpFieldFound) {
            ...
            if (curUrl.includes('verify') || curUrl.includes('otp') || curUrl.includes('confirmation') || curUrl.includes('code')) {
                otpFieldFound = true;
                log('INFO', `OTP page detected from URL: ${curUrl}`, accountNum);
            }
        }
        ...
        if (otpFieldFound && config.email_imap) {
            ...
            try {
                ...
                // Fill OTP — try explicit selectors first, then any visible text input
                let otpFilled = false;
                for (const sel of otpSelectors) {
                    try {
                        const el = await page.$(sel);
                        if (el && await el.isVisible()) {
                            await el.fill(otp);
                            log('INFO', `OTP filled via: ${sel}`, accountNum);
                            otpFilled = true;
                            break;
                        }
                    } catch (e) {}
                ...
                // Fallback: fill the first visible text/tel/number input (OTP page has very few inputs)
                if (!otpFilled) {
                    try {
                        ...
                        for (const inp of inputs) {
                            if (await inp.isVisible()) {
                                ...
                                if (!type || type === 'text' || type === 'tel' || type === 'number') {
                                    await inp.fill(otp);
                                    log('INFO', 'OTP filled via first visible input', accountNum);
                                    otpFilled = true;
                                    break;
                                }
                            }
...
async function main() {
    ...
    // Process accounts SEQUENTIALLY — one at a time
    const delayBetween = (config.timing && config.timing.delay_between_accounts) || 5000;
    const results = [];
    const pin = config.create.default_pin || '112233';

    log('INFO', `\n========================================`);
    log('INFO', `  SEQUENTIAL MODE: Creating ${accounts.length} accounts one by one`);
    log('INFO', `========================================\n`);

    for (let i = 0; i < accounts.length; i++) {
        const accountNum = i + 1;
        log('INFO', `\n========================================`);
        log('INFO', `  ACCOUNT ${accountNum} of ${accounts.length}`);
        log('INFO', `  Email: ${accounts[i].email}`);
        log('INFO', `========================================`);

        let result;
        try {
            result = await createAccount(browser, config, accounts[i], accountNum, proxies, nopecha);
        } catch (err) {
            result = { success: false, email: accounts[i].email, error: err.message };
        }

        results.push(result);

        if (result.success) {
            log('INFO', `\n  *** SUCCESS — Account #${accountNum} ***`);
            log('INFO', `  Email: ${result.email}`);
            log('INFO', `  PIN:   ${pin}`);
            log('INFO', `  *************************************\n`);
        } else {
            log('WARNING', `\n  *** FAILED — Account #${accountNum} ***`);
            log('WARNING', `  Email: ${result.email}`);
            log('WARNING', `  Error: ${result.error || 'unknown'}`);
            log('WARNING', `  *************************************\n`);
        }

        // Save results after each account (so progress is not lost)
        const successSoFar = results.filter(r => r.success).length;
        const failedSoFar = results.length - successSoFar;
        log('INFO', `Progress: ${successSoFar} succeeded, ${failedSoFar} failed, ${accounts.length - results.length} remaining`);

        fs.writeFileSync('creation_results.json', JSON.stringify(results, null, 2));
        ...
        if (successfulAccounts.length > 0) {
            fs.writeFileSync('created_accounts.json', JSON.stringify(successfulAccounts, null, 2));
        }

        // Delay before next account
        if (i < accounts.length - 1) {
            log('INFO', `Waiting ${delayBetween / 1000}s before next account...`);
    ...
    if (successful > 0) {
        ...
        results.filter(r => r.success).forEach((r, idx) => {
            log('INFO', `    ${idx + 1}. ${r.email} / PIN: ${pin}`);
        });
    }

    log('INFO', `\n  Results saved to: creation_results.json`);
    log('INFO', `  Accounts saved to: created_accounts.json`);
